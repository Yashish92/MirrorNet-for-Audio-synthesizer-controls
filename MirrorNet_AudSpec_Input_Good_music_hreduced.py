...
class decoder(nn.Module):
    def __init__(self):
        ...
        self.L1 = nn.Sequential(
            nn.Conv1d(16, 128, kernel_size=1)  # changed 1 + 513 * 2 to 16
        )
        self.CNN = nn.ModuleList([])
        for s in range(self.stack):
            self.CNN.append(CNN1D(self.BN_channel, self.BN_channel, self.kernel,
...
def quickEval(epoch, ideal_h=False, train_E=True, train_D=True, loader="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        batch_spec = Variable(data[3]).contiguous().float()  # We use the spec of the original music wave file
        ...
        music_temp = batch_h[:, 0:16, :].data.cpu().numpy().astype('float64')   # Doubtful about the shape
    ...
    for (batch_idx, data_random), (_, data_train) in zip(enumerate(train_random), enumerate(train_loader)):
        ...
        if train_D and not init:
            ...
            for j in range(music_tmp.shape[0]):
                ...
                y = music_syn.music_synthesize(music.mean(axis=0), fs)
                y = y[pad:pad + 32000]
...
def generate_figures(mode="evaluation", name="", load_weights=("", "")):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        # Save ap
        # Save Sp
        music_hat_saved = np.zeros((num, 16, 1), dtype=np.float64)
        # sp_hat_saved = np.zeros((num, 513, 251), dtype=np.float64)
        music_ideal_saved = np.zeros((num, 16, 1), dtype=np.float64)
...
parser.add_argument('--train_data', type=str, default=base_dir + "music_data_hr/train.data",
                    help='path to training data')

parser.add_argument('--test_data', type=str, default=base_dir + "music_data_hr/test.data",
                    help='path to testing data')

parser.add_argument('--initialization_data', type=str, default=base_dir + "music_data_hr/initialize.data",
                    help='path to initialization data')

parser.add_argument('--train_random_data', type=str, default=base_dir + "music_data_hr/train_random.data",
...
# trainTogether_newTechnique(epochs=300, init=True, train_D=True, train_E=True, loader_eval="train", save_better_model=True)
...
print("POST_TRAINING")
trainTogether_newTechnique(epochs=5, init=True, train_D=True, train_E=False, loader_eval="train",
                           save_better_model=True)  # 100 to 5

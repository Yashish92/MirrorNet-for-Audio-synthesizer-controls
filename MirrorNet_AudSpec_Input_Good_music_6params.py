...
class decoder(nn.Module):
    def __init__(self):
        ...
        self.L1 = nn.Sequential(
            nn.Conv1d(6, 128, kernel_size=1)  # changed 1 + 513 * 2 to 16
        )
        self.CNN = nn.ModuleList([])
        for s in range(self.stack):
            self.CNN.append(CNN1D(self.BN_channel, self.BN_channel, self.kernel,
...
def quickEval(epoch, ideal_h=False, train_E=True, train_D=True, loader="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        music_temp = batch_h[:, 6:, :].data.cpu().numpy().astype('float64')   # Doubtful about the shape
    ...
    for (batch_idx, data_random), (_, data_train) in zip(enumerate(train_random), enumerate(train_loader)):
        ...
        if train_D and not init:
            ...
            for j in range(music_tmp.shape[0]):
                ...
                music = music_tmp[j, :, :].T.copy(order='C')  # 406, 513


                y = music_syn.music_synthesize(music.mean(axis=0), fs, engine, generator, rev_idx)
                ...
                spec_wav[j, :, 0:250] = np.sqrt(spec_h_temp)[:,0:250]
...
def trainTogether_newTechnique(epochs=None, save_better_model=False, loader_eval="evaluation", train_E=False,
    ...
    for epoch in range(1, epochs + 1):
        ...
        if train_D and epoch % 5 == 0:      #chnged 5 to 1
            print("EVAL")
        ...
        if train_E and epoch % 5 == 0:      #chnged 5 to 1
            print("EVAL")
            total_loss_eval, encoder_loss_eval, decoder_loss_eval = quickEval(epoch, loader='evaluation',
                                                                              train_D=train_D, train_E=train_E,
                                                                              ideal_h=True)
        ...
        if train_D and epoch % 10 == 0:      # changed 10 to 1
    ...
    if train_D:
        ...
        plt.title("Evolution of the difference between D(h_hat) and W(h_hat) during training")
        plt.plot(new_loss)
        plt.savefig(out + "/loss/loss_" + name + ".eps")
        plt.close()

    if train_D:
        plt.title("Decoder Loss Eval")
        plt.plot(decoder_loss_eval)
        plt.savefig(out + "/loss/decoder_loss_eval_" + name + ".eps")
        plt.close()

    if train_E:
        plt.title("Encoder Loss Eval")
        plt.plot(encoder_loss_eval)
        plt.savefig(out + "/loss/encoder_loss_eval_" + name + ".eps")
        plt.close()
...
def generate_figures(mode="evaluation", name="", load_weights=("", "")):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        music_temp = batch_h_hat[:, 0:6, :].data.cpu().numpy().astype('float64')
        # f0_temp = batch_h[:,0:1,:].data.cpu().numpy().astype('float64')
        ...
        for j in range(music_temp.shape[0]):
            ...
            # sp = sp_temp[j, :, :].T.copy(order='C')  # 406, 513
            # sp = np.exp(sp)  # from log scale to original scale required by WORLD
            music = music_temp[j, :, :].T.copy(order='C')  # 406, 513
            ...
            # spec_tmp[spec_tmp == float("-Inf")] = -50
            # spec_wav[j,:,:] = spec_tmp
            spec_wav[j, :, 0:250] = np.sqrt((nsl.wav2aud(nsl.unitseq(y), paras_c)).T)[:,0:250]
        ...
        music = batch_h[:, 0:6, :].data.cpu().numpy().astype('float64')
        # f0 = batch_h[:,0:1,:].data.cpu().numpy().astype('float64')
        ...
        # local_out_init += "/lentent_space/"
        # print (f0.shape[0])
        num = np.min((music.shape[0], 10))
        # print (num, 'num')
        # Save ap
        # Save Sp
        music_hat_saved = np.zeros((num, 6, 251), dtype=np.float64)
        # sp_hat_saved = np.zeros((num, 513, 251), dtype=np.float64)
        music_ideal_saved = np.zeros((num, 6, 251), dtype=np.float64)
        ...
        for i in range(min(music.shape[0], 10)):
            ...
            plt.colorbar()
            plt.savefig(local_out + "/music.eps")
...
def getSpectrograms(mode="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        # f0 = batch_h[:, 0:1, :].data.cpu().numpy().astype('float64')
        # sp = batch_h[:, 1:514, :].data.cpu().numpy().astype('float64')
        music = batch_h[:, 6:, :].data.cpu().numpy().astype('float64')
...
parser.add_argument('--train_data', type=str, default=base_dir + "music_data_6params_new/train_audio.data",
                    help='path to training data')

parser.add_argument('--test_data', type=str, default=base_dir + "music_data_6params_new/test_audio.data",
                    help='path to testing data')

parser.add_argument('--initialization_data', type=str, default=base_dir + "music_data_6params_new/initialize_audio.data",
                    help='path to initialization data')

parser.add_argument('--train_random_data', type=str, default=base_dir + "music_data_6params_new/train_audio_random.data",
...
E_scheduler = torch.optim.lr_scheduler.ExponentialLR(E_optimizer, gamma=0.3) # changed gamma=0.5 to 0.3

D_optimizer = optim.Adam(D.parameters(), lr=args.decoder_lr)
D_scheduler = torch.optim.lr_scheduler.ExponentialLR(D_optimizer, gamma=0.3) # changed gamma=0.5 to 0.1

parameters = [p for p in E.parameters()] + [p for p in D.parameters()]
all_optimizer = optim.Adam(parameters, lr=args.lr)
all_scheduler = torch.optim.lr_scheduler.ExponentialLR(all_optimizer, gamma=0.3)  # changed gamma=0.5 to 0.2
...
engine, generator, rev_idx = music_syn.create_synth('toy')
...
trainTogether_newTechnique(epochs=150, init=True, train_D=True, train_E=True, loader_eval="train",
                           save_better_model=False)
...
for i in range(10): 
    ...
    trainTogether_newTechnique(epochs=20, name="train_D_" + str(i + 1), init=False, train_D=True, train_E=False,
                               loader_eval="train", save_better_model=False)  # 20 to 5
    trainTogether_newTechnique(epochs=40, name="train_E_" + str(i + 1), init=False, train_D=False, train_E=True,

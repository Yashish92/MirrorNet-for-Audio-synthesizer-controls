...
if recon_from_latent:
    ...
    for speaker_index in range(0, 10):
        ...
        music_ideal = ap_ideal[:, :].T.copy(order='C')  # 406, 513

        # f0_hat = np.squeeze(f0_hat[speaker_index, :, :])
        ap_hat = np.squeeze(ap_hat[speaker_index, :, :])
        music_hat = ap_hat[:, :].T.copy(order='C')  # 406, 513
        ...
        y_ideal = music_syn.music_synthesize(music_ideal.mean(axis=0), fs, engine, generator, rev_idx)
        ...
        y_hat = music_syn.music_synthesize(music_hat.mean(axis=0), fs, engine, generator, rev_idx)

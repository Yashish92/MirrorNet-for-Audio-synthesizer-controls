...
class encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(len(self.CNN)):
            this_input = self.CNN[i](enc_output)
	...
	#music_arr[:, 0, :] = (music_arr[:, 0, :] * 12) +62
	#music_arr[:, 1, :] = (music_arr[:, 1, :] * 0.3) + 0.7
	...
	#music_arr[:, 3, :] = (music_arr[:, 3, :] * 50) + 50
        ...
        #return Variable(music, requires_grad = True)
    ...
    for (batch_idx, data_random), (_, data_train) in zip(enumerate(train_random), enumerate(train_loader)):
        ...
        if train_D and not init:
            ...
            for j in range(music_tmp.shape[0]):
                ...
                y = music_syn.generate(music, engine, generator, parameters, rev_idx, pad=0)
                ...
                spec_wav[j, :, 0:spec_len] = np.sqrt(spec_h_temp)[:,0:spec_len]
...
def generate_figures(mode="evaluation", name="", load_weights=("", "")):
	...
	#music_temp[:, 3, :] = (music_temp[:, 3, :] * 50) + 50
        ...
        for j in range(music_temp.shape[0]):
            ...
            # spec_tmp[spec_tmp == float("-Inf")] = -50
            # spec_wav[j,:,:] = spec_tmp
            spec_wav[j, :, :] = np.sqrt((nsl.wav2aud(nsl.unitseq(y), paras_c)).T)[:,0:spec_len]
...
def getSpectrograms(mode="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        for j in range(music_temp.shape[0]):
            ...
            spec_wav[j, :, 0:spec_len] = (np.sqrt((nsl.wav2aud(nsl.unitseq(y), paras_c)).T))[:, 0:spec_len]
...
def plotSpectrograms(spectrograms, name="", MAX_examples=10):
    ...
    for i in range(min(len(spectrograms[0][0]), MAX_examples)):
        ...
        np.save("spectrogram_{}.mat".format(i),modelSpectrogram[i])
...
parser.add_argument('--train_data', type=str, default=base_dir + "music_data_6params_new/train_audio_new3.data",
                    help='path to training data')

parser.add_argument('--test_data', type=str, default=base_dir + "music_data_6params_new/test_audio_new3.data",
...
parser.add_argument('--train_random_data', type=str, default=base_dir + "music_data_6params_new/train_audio_new3_random.data",
...
for i in range(4): 
    ...
    trainTogether_newTechnique(epochs=25, name="train_D_" + str(i + 1), init=False, train_D=True, train_E=False,
                               loader_eval="train", save_better_model=False)  # 20 to 5
    trainTogether_newTechnique(epochs=100, name="train_E_" + str(i + 1), init=False, train_D=False, train_E=True,

...
import music_synthesize_128params as music_syn
...
class decoder(nn.Module):
    def __init__(self):
        ...
        self.L1 = nn.Sequential(
            nn.Conv1d(128, 128, kernel_size=1)  # changed 1 + 513 * 2 to 16 to 128
        )
        self.CNN = nn.ModuleList([])
        for s in range(self.stack):
            self.CNN.append(CNN1D(self.BN_channel, self.BN_channel, self.kernel,
...
def quickEval(epoch, ideal_h=False, train_E=True, train_D=True, loader="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        music_temp = batch_h[:, 128:, :].data.cpu().numpy().astype('float64')   # Doubtful about the shape
    ...
    for (batch_idx, data_random), (_, data_train) in zip(enumerate(train_random), enumerate(train_loader)):
        ...
        if train_D and not init:
            ...
            for j in range(music_tmp.shape[0]):
                ...
                y = y[pad:pad + 32000]
                # y = y / np.sqrt(np.sum(y ** 2.0))  # normalize the power of waveform to 1
                # print(np.sqrt(np.sum(y ** 2.0)))
...
def generate_figures(mode="evaluation", name="", load_weights=("", "")):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        music_temp = batch_h_hat[:, 0:128, :].data.cpu().numpy().astype('float64')
        # f0_temp = batch_h[:,0:1,:].data.cpu().numpy().astype('float64')
        ...
        music = batch_h[:, 0:128, :].data.cpu().numpy().astype('float64')
        # f0 = batch_h[:,0:1,:].data.cpu().numpy().astype('float64')
        ...
        # Save ap
        # Save Sp
        music_hat_saved = np.zeros((num, 128, 251), dtype=np.float64)
        # sp_hat_saved = np.zeros((num, 513, 251), dtype=np.float64)
        music_ideal_saved = np.zeros((num, 128, 251), dtype=np.float64)
...
def getSpectrograms(mode="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        for j in range(music_temp.shape[0]):
            ...
            # spec_tmp = np.log(spec_tmp)
            # spec_tmp[spec_tmp == float("-Inf")] = -50
            spec_wav[j, :, 0:250] = np.sqrt(spec_tmp)[:,0:250]

        # print("out side initialization for loop")

        # f0 = batch_h[:, 0:1, :].data.cpu().numpy().astype('float64')
        # sp = batch_h[:, 1:514, :].data.cpu().numpy().astype('float64')
        music = batch_h[:, 128:, :].data.cpu().numpy().astype('float64')
...
parser.add_argument('--train_data', type=str, default=base_dir + "music_data_128params/train_audio.data",
                    help='path to training data')

parser.add_argument('--test_data', type=str, default=base_dir + "music_data_128params/test_audio.data",
                    help='path to testing data')

parser.add_argument('--initialization_data', type=str, default=base_dir + "music_data_128params/initialize_audio.data",
                    help='path to initialization data')

parser.add_argument('--train_random_data', type=str, default=base_dir + "music_data_128params/train_audio_random.data",
...
for i in range(6):  # 15 to 3
    ...
    trainTogether_newTechnique(epochs=20, name="train_E_" + str(i + 1), init=False, train_D=False, train_E=True,

...
fs=sampling_rate
figs_folder='NEW_with_only_1_loss_for_E_DATE_2021-01-10_H_1'
...
if recon_from_latent:
	...
	with open('./figs/'+figs_folder+'/init_evaluation/lentent_space/ap_ideal.pkl', 'rb') as f:
		ap_ideal=pkl.load(f)

	with open('./figs/'+figs_folder+'/init_evaluation/lentent_space/ap_hat.pkl', 'rb') as f:
		ap_hat=pkl.load(f)
	...
	y_ideal = pw.synthesize(f0_ideal, (np.ascontiguousarray(sp_ideal.T)), np.ascontiguousarray(ap_ideal.T), sampling_rate, frame_period=8.0)
	y_ideal = y_ideal/np.sqrt(np.sum(y_ideal**2)) 
	y_ideal=nsl.unitseq(y_ideal)
	scipy.io.wavfile.write(name+'y_ideal_latent_recon.wav' ,16000,y_ideal)


	# np.ascontiguousarray(sp_hat)
	# np.ascontiguousarray(ap_hat)
	# np.ascontiguousarray(f0_hat)
	y_hat = pw.synthesize(f0_hat, (np.ascontiguousarray(sp_hat.T)), np.ascontiguousarray(ap_hat.T), sampling_rate, frame_period=8.0)

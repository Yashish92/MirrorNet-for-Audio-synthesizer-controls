...
#import music_synthesize_violin_extended as music_syn
import json
import re

np.set_printoptions(threshold=sys.maxsize)

def generate_data(path_audio, audio_time=2, sampling_rate=8000, random=False):   # changed sampling rate to 8000 from 4000
    ...
    directory = path_audio[:-1] + "_piano_8khz_minus2.data"  # save data to this path
    if random:
    ...
    param_array = np.zeros(no_of_params)  # for music synthesizer preset
   ...
   #         if '.npy' in files:
   #             if random:
   #                     file_no = int(re.split('_|\.',files)[0])
   #             else:
   #                     file_no = int(re.split('_|\.',files)[0])
   #             # print(dirnames)
   ...
   # print(h_tmp[2, :, :])
    ...
    frmlen = 8
    tc = 8
    paras_c = [frmlen, tc, -2, np.log2(sampling_rate / 16000.0)]   # changed compression from 0.05
    # print(sampling_rate)

...
def generate_data(path_audio, path_params, audio_time=2, sampling_rate=16000, random=False):
    ...
    directory = path_audio[:-1] + ".data"  # save data to this path
    if random:
        directory = path_audio[:-1] + "_random" + ".data"  # save data to this path
    ...
    param_array = np.zeros(16)      # for music synthesizer preset
    spk_tmp = np.zeros((spk_wav_tmp.shape[0], spk_wav_tmp.shape[1]))  # raw speech with normalized power
    h_tmp = np.zeros((spk_tmp.shape[0],no_of_params, 251))  # ideal hiddens from world
    spec_tmp_513 = np.zeros(
        (spk_wav_tmp.shape[0], 128, 251))  # dimensions of the AudSpec - needs to be softcoded for scalability
    spec_tmp_513_pw = np.zeros((spk_wav_tmp.shape[0], 128,
                                251))  # dimensions of the Reconstructed AudSpec - needs to be softcoded for scalability
    ...
    for (dirpath, dirnames, filenames) in tqdm(os.walk(path_params)):
        for files in tqdm(filenames):
            if '.npz' in files:
                ...
                param_array = np.reshape(param_array, (16,1))
                extended_arr = np.tile(param_array, (1,251))
                h_tmp[i, 0:16, :] = extended_arr
                i += 1
    ...
    hid_set = dset.create_dataset('hidden', shape=(spk_tmp.shape[0], no_of_params, 251), dtype=np.float64)

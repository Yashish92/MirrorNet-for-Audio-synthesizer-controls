    ...
    for (batch_idx, data_random), (_, data_train) in zip(enumerate(train_random), enumerate(train_loader)):
        ...
        if train_D and not init:
            ...
            for j in range(music_tmp.shape[0]):
                ...
                y = music_syn.music_synthesize(music, fs, engine, generator, rev_idx)
...
def generate_figures(mode="evaluation", name="", load_weights=("", "")):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        # Save ap
        # Save Sp
        music_hat_saved = np.zeros((num, 6, 1), dtype=np.float64)
        # sp_hat_saved = np.zeros((num, 513, 251), dtype=np.float64)
        music_ideal_saved = np.zeros((num, 6, 1), dtype=np.float64)
...
parser.add_argument('--train_data', type=str, default=base_dir + "music_data_6params_new/train_audio_1s_hreduced.data",
                    help='path to training data')

parser.add_argument('--test_data', type=str, default=base_dir + "music_data_6params_new/test_audio_1s_hreduced.data",
...
parser.add_argument('--train_random_data', type=str,
                    default=base_dir + "music_data_6params_new/train_audio_random_1s_hreduced.data",
...
all_scheduler = torch.optim.lr_scheduler.ExponentialLR(all_optimizer, gamma=0.1)  # changed gamma=0.5 to 0.2
...
for i in range(2):
    ...
    trainTogether_newTechnique(epochs=5, name="train_D_" + str(i + 1), init=False, train_D=True, train_E=False,
                               loader_eval="train", save_better_model=False)  # 20 to 5
    trainTogether_newTechnique(epochs=5, name="train_E_" + str(i + 1), init=False, train_D=False, train_E=True,

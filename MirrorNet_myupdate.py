...
class decoder(nn.Module):
    def __init__(self):
        ...
        self.f0_seq = nn.Sequential(
            ...
            nn.BatchNorm1d(64, 64, 1),
            nn.ReLU(),
            nn.Conv1d(64, 128, 1),  # Kernel size 1 with 0 padding maintains length of output signal=401
        ...
        self.sp_seq = nn.Sequential(
            ...
            nn.BatchNorm1d(256, 256, 1),
            nn.ReLU(),
            nn.Conv1d(256, 128, 1),  # Kernel size 1 with 0 padding maintains length of output signal=401
            nn.BatchNorm1d(128, 128, 1),
            nn.ReLU()
    ...
    def forward(self, input):
        ...
        sp_out = self.sp_seq(input[1])
        ap_out = self.ap_seq(input[2])

        this_input = [f0_out, sp_out, ap_out].detach().cpu().numpy()
        this_input = np.concatenate([f0_out, sp_out, ap_out], 1)

        new_input = nn.Conv1d(128*3, 256, 1)(this_input)

        # print (this_input.size(),'this_input before CNN stack decoder--------' )
        for i in range(len(self.CNN)):
            this_input = self.CNN[i](new_input)  # 64-128-250
...
def quickEval(epoch, ideal_h=False, train_E=True, train_D=True, loader="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        if train_D:
            s1 = D([batch_f0, batch_sp, batch_ap])
            # DECODER
        ...
        if train_E:
            if not ideal_h:
                s1_pred = D([batch_f0, batch_sp, batch_ap])
                # ENCODER
...
def new_training_technique(epoch, train_D=False, train_E=False, init=False):
    ...
    for (batch_idx, data_random), (_, data_train) in zip(enumerate(train_random), enumerate(train_loader)):
        ...
        if train_D:
            ...
            if init:
                loss2 = criteron(D([batch_h_random[:, 0, :], batch_h_random[:, 1:514, :], batch_h_random[:, 514:, :]]), batch_spec_random)
                loss2.backward()

            else:
                # loss_D1 = criteron(D(batch_h_random), batch_spec_random)
                loss_D2 = criteron(D([batch_f0, batch_sp, batch_ap]), s2)
        ...
        if train_E:
            ...
            if init:
                ...
                loss_E1 = criteron(D([batch_f0, batch_sp, batch_ap]), batch_spec_train)
                # a, b, c = E(batch_spec_random)
            ...
            if save_better_model:
                ...
                if train_E:
                    ...
                    if epoch >= 10 and np.min(validation_loss_encoder) == validation_loss_encoder[-1]:
                        with open(base_dir + exp_new + '/net/rahil_model_weight_E1.pt', 'wb') as f:
                ...
                if train_D:
                    ...
                    if epoch >= 10 and np.min(validation_loss_decoder) == validation_loss_decoder[-1]:
                        with open(base_dir + exp_new + '/net/rahil_model_weight_D1.pt', 'wb') as f:
                            torch.save(D.cpu().state_dict(), f)
                        print("We saved decoder!")
                        print()

    if save_better_model and train_D:
        D.load_state_dict(torch.load(base_dir + exp_new + '/net/rahil_model_weight_D1.pt'))
    if save_better_model and train_E:
        E.load_state_dict(torch.load(base_dir + exp_new + '/net/rahil_model_weight_E1.pt'))
...
def generate_figures(mode="evaluation", name="", load_weights=("", "")):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        for i in range(min(f0.shape[0], 10)):
            ...
            ax1 = plt.subplot(211)
            plt.title("D(h_hat)")
            plt.imshow(D([batch_f0, batch_sp, batch_ap]).detach().cpu().numpy()[i, :, :], cmap='jet', aspect="auto")
            plt.colorbar()
            ...
            plt.title("original")
            plt.subplot(122)
            plt.imshow(D([batch_f0, batch_sp, batch_ap]).detach().cpu().numpy()[i, :, :], cmap='jet', origin='lower')
            plt.title("generated")
            plt.colorbar()
...
def getSpectrograms(mode="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        decoderSpectrogram = np.array(D([batch_f0, batch_sp, batch_ap]).detach().cpu().numpy())
        modelSpectrogram = np.array(D([batch_f0, batch_sp, batch_ap]).detach().cpu().numpy())
...
parser.add_argument('--train_data', type=str,  default=base_dir+"data/"+"train.data",
                    help='path to training data')

parser.add_argument('--test_data', type=str,  default=base_dir+"data/female_voice.data",
...
parser.add_argument('--train_random_data', type=str,  default=base_dir+"data/train_random.data",
                    help='path to train random data')


# parser.add_argument('--train_data', type=str,
#                     default=base_dir + "data/" + "train_set_male_speakers_augmented_v2_0909.data",
#                     help='path to training data')

# parser.add_argument('--train_data', type=str,  default=base_dir+"data/"+"WSJ_00F_train.data",
#                     help='path to training data')
# parser.add_argument('--test_data', type=str,  default=base_dir+"data/test_set_287_0909.data",
#                    help='path to testing data')

# parser.add_argument('--test_data', type=str, default=base_dir + "data/WSJ_male_test.data",
#                     help='path to testing data')
...
# parser.add_argument('--train_random_data', type=str,  default=base_dir+"data/train_set_male_speakers_augmented_v2_0909_flip_random.data",
#                    help='path to train random data')

# parser.add_argument('--train_random_data', type=str,
#                     default=base_dir + "data/train_set_male_speakers_augmented_v2_0909female_train_sp_random.data",
#                     help='path to train random data')
...
# trainTogether_newTechnique(epochs=300, init=True, train_D=True, train_E=True, loader_eval="train", save_better_model=True, name='init')
...
for i in range(12):
    ...
    trainTogether_newTechnique(epochs=19, name="train_D_" + str(i + 1), init=False, train_D=True, train_E=False,
                               loader_eval="train", save_better_model=False)
    trainTogether_newTechnique(epochs=15, name="train_E_" + str(i + 1), init=False, train_D=False, train_E=True,
                               loader_eval="train", save_better_model=False)
    if i % 2 == 0:
...
print("POST_TRAINING")
trainTogether_newTechnique(epochs=1, init=True, train_D=True, train_E=False, loader_eval="train",
                           save_better_model=False, name='POST_TRAINING')

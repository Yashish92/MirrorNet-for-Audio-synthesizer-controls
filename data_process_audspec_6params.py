...
def generate_data(path_audio, path_params, audio_time=1, sampling_rate=4000, random=False):   # changed audio time to 1 from 2
    ...
    directory = path_audio[:-1] + "_1s.data"  # save data to this path
    if random:
        directory = path_audio[:-1] + "_random" + "_1s.data"  # save data to this path
    ...
    h_tmp = np.zeros((spk_tmp.shape[0], no_of_params, 126))  # ideal hiddens from world
    ...
    for (dirpath, dirnames, filenames) in tqdm(os.walk(path_params)):
        for files in tqdm(filenames):
            if '.npz' in files:
                ...
                # print(extended_arr)
                h_tmp[i, :, :] = extended_arr.copy(order='C')
    ...
    hid_set = dset.create_dataset('hidden', shape=(spk_tmp.shape[0], no_of_params, 126), dtype=np.float64)

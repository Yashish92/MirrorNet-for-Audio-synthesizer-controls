...
def generate_data(path_audio, path_params, audio_time=2, sampling_rate=4000, random=False):   # changed audio time to 1 from 2
    ...
    param_array = np.zeros(4)  # for music synthesizer preset
    ...
    for (dirpath, dirnames, filenames) in tqdm(os.walk(path_params)):
        for files in tqdm(filenames):
            if '.npy' in files:
                ...
                # print(extended_arr)
                h_tmp[i, :, :] = loaded_params.copy(order='C')

...
def create_synth(dataset, path='Diva.vst'):
...
def write_to_wav(audio, file_name):
	...
	scipy.io.wavfile.write(my_path + '/' + file_name + '.wav', 44100, (np.iinfo(np.int16).max*audio/np.max(audio)).astype(np.int16))

def generate(params, outName, pad=0):
	'''
...
if __name__ == "__main__":
	...
	for i in range(10):
		...
		duration = 4*np.array(duration)/np.sum(duration) - pad
		generate([pitch, volume, duration, filterCutOff], "test_"+str(i), pad=pad)

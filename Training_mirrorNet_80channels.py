...
class encoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Conv1d(80, self.AE_channel, 1)
...
class decoder(nn.Module):
    def __init__(self):
        ...
        self.L1 = nn.Sequential(
            nn.Conv1d(16, 80, kernel_size=1)  # changed 1 + 513 * 2 to 16
        )
        self.CNN = nn.ModuleList([])
        for s in range(self.stack):
            self.CNN.append(CNN1D(self.BN_channel, self.BN_channel, self.kernel,
    ...
    def forward(self, input):
        ...
        this_input = self.L1(input)  # 64-80-250
        # print (this_input.size(),'this_input before CNN stack decoder--------' )
        for i in range(len(self.CNN)):
            this_input = self.CNN[i](this_input)  # 64-80-250
...
def quickEval(epoch, ideal_h=False, train_E=True, train_D=True, loader="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        music_temp = batch_h[:, 16:, :].data.cpu().numpy().astype('float64')   # Doubtful about the shape
...
def new_training_technique(epoch, train_D=False, train_E=False, init=False):
    ...
    for (batch_idx, data_random), (_, data_train) in zip(enumerate(train_random), enumerate(train_loader)):
        ...
        batch_music = E(batch_spec_train)
        ...
        if train_D and not init:
            ...
            music_tmp = batch_music.data.cpu().numpy().astype('float64')
            print(music_tmp.shape[0])

            spec_wav = np.zeros((batch_wav_train.shape[0], 80, 251)).astype('float64')
            ...
            for j in range(music_tmp.shape[0]):
                ...
                spec_wav[j, :, 0:250] = np.sqrt(spec_h_temp)[0:80,0:250]
...
def trainTogether_newTechnique(epochs=None, save_better_model=False, loader_eval="evaluation", train_E=False,
    ...
    for epoch in range(1, epochs + 1):
        ...
        if train_D and epoch % 5 == 0:      # changed 10 to 1
...
def generate_figures(mode="evaluation", name="", load_weights=("", "")):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        music_temp = batch_h_hat[:, 0:16, :].data.cpu().numpy().astype('float64')
        # f0_temp = batch_h[:,0:1,:].data.cpu().numpy().astype('float64')
        ...
        spec_wav = np.zeros((batch_wav.shape[0], 80, 251)).astype('float64')
        ...
        for j in range(music_temp.shape[0]):
            ...
            # spec_tmp[spec_tmp == float("-Inf")] = -50
            # spec_wav[j,:,:] = spec_tmp
            spec_wav[j, :, 0:250] = np.sqrt((nsl.wav2aud(nsl.unitseq(y), paras_c)).T)[0:80,0:250]

        # f0 = batch_h[:, 0:1, :].data.cpu().numpy().astype('float64')
        # sp = batch_h[:, 1:514, :].data.cpu().numpy().astype('float64')
        music = batch_h[:, 0:16, :].data.cpu().numpy().astype('float64')
        # f0 = batch_h[:,0:1,:].data.cpu().numpy().astype('float64')
        ...
        # Save ap
        # Save Sp
        music_hat_saved = np.zeros((num, 16, 251), dtype=np.float64)
        # sp_hat_saved = np.zeros((num, 513, 251), dtype=np.float64)
        music_ideal_saved = np.zeros((num, 16, 251), dtype=np.float64)
...
def getSpectrograms(mode="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        for j in range(music_temp.shape[0]):
            ...
            # spec_tmp = np.log(spec_tmp)
            # spec_tmp[spec_tmp == float("-Inf")] = -50
            spec_wav[j, :, 0:250] = np.sqrt(spec_tmp)[0:80,0:250]

        # print("out side initialization for loop")

        # f0 = batch_h[:, 0:1, :].data.cpu().numpy().astype('float64')
        # sp = batch_h[:, 1:514, :].data.cpu().numpy().astype('float64')
        music = batch_h[:, 16:, :].data.cpu().numpy().astype('float64')
...
parser.add_argument('--train_data', type=str, default=base_dir + "music_data_extended/train_audio.data",
                    help='path to training data')

parser.add_argument('--test_data', type=str, default=base_dir + "music_data_extended/test_audio.data",
                    help='path to testing data')

parser.add_argument('--initialization_data', type=str, default=base_dir + "music_data_extended/initialize_audio.data",
                    help='path to initialization data')

parser.add_argument('--train_random_data', type=str, default=base_dir + "music_data_extended/train_audio_random.data",
...
trainTogether_newTechnique(epochs=200, init=True, train_D=True, train_E=True, loader_eval="train",
...
print("POST_TRAINING")
trainTogether_newTechnique(epochs=100, init=True, train_D=True, train_E=False, loader_eval="train",

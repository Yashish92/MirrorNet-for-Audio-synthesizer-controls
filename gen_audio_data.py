...
def resample(y, orig_sr, target_sr):
    ...
    y_hat = scipy.signal.resample(y, n_samples, axis=-1)  # maybe resample is better?
...
def music_synthesize(passed_params, file_name, engine, generator, param_defaults, rev_idx):
    # print(passed_params)
...
def gen_json_list(param_passed):
    ...
    for i in range(0,6):
        cu_val = param_passed_list[final_params[i]]
        param_list[final_params[i]] = cu_val
    #print(param_list)
    return param_list

def write_to_wav(audio, file_name):
    final_audio = resample(audio, 44100, 22050)
    librosa.output.write_wav(my_path +"/diva_raw/test_audio/" + file_name + '.wav', final_audio, 22050)
...
if __name__ == "__main__":
    ...
    for file in os.listdir("diva_raw/test_params"):
        if file.endswith(".npz"):
            loaded = np.load("diva_raw/test_params/"+file, allow_pickle=True)
            loaded_params, loaded_chars, loaded_audio = loaded["param"], loaded["chars"], loaded["audio"]
        music_synthesize(loaded_params, file, engine, generator, param_defaults, rev_idx)

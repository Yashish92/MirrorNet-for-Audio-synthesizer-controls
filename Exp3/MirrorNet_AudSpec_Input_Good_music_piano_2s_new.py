...
params = 7
pad_val = 0.1
diva_drive_params = 7 #5
down_sample_rate = 8000.0
...
class BoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return wav_tensor, [], specWorld_tensor
...
def quickEval(epoch, ideal_h=False, train_E=True, train_D=True, loader="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        batch_spec = Variable(data[2]).contiguous().float()  # We use the spec of the resynthesized spec
...
def new_training_technique(epoch, train_D=False, train_E=False, init=False):
    ...
    for (batch_idx, data_train) in enumerate(train_loader):
        ...
        if train_D and not init:
            ...
            frmlen = 8
            tc = 8
            sampling_rate = down_sample_rate
            ...
            for j in range(music_tmp.shape[0]):
                ...
                y = music_syn.generate(music, engine, generator, parameters, rev_idx, pad=pad_val, drive_params=diva_drive_params)
                y = music_syn.resample(y, 44100, fs)  # changed 44100
                y = y[pad:pad + 32000]
...
def generate_figures(mode="evaluation", name="", load_weights=("", "")):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        batch_wav = Variable(data[0]).contiguous().float()
        #batch_h = Variable(data[2]).contiguous().float()
        batch_spec = Variable(data[2]).contiguous().float()
        ...
        for j in range(music_temp.shape[0]):
            ...
            y = music_syn.generate(music, engine, generator, parameters, rev_idx, pad=pad_val, drive_params=diva_drive_params)
            y = music_syn.resample(y, 44100, fs)
            #print(y.shape)
            y = y[pad:pad + 32000]
            #print(y.shape)
            y = y / np.sqrt(np.sum(y ** 2))  # normalize the power of waveform to 1

            # Uncomment to plot the waveforms
            #plt.title("Waveform")
            #plt.subplot(221)
        ...
        # local_out_init += "/lentent_space/"
        # print (f0.shape[0])
        num = np.min((spec_wav.shape[0], 10))
        # print (num, 'num')
        # Save ap
        # Save Sp
        ...
        for i in range(num):
            ...
            #batch_wav_saved[i, :, :] = batch_wav[i].cpu().numpy()
        ...
        for i in range(min(music_temp.shape[0], 10)):
            ...
            #fig.title("Music Parms")
            fig,ax= plt.subplots()
            ax.set_title("from encoder (E(x))")
            img = ax.imshow(music_temp[i, :, :], cmap=plt.cm.BuPu_r)
            ax.set_yticklabels(params_labels)
            ax.set_title("Music Parameters (0-1 range)")
            #plt.subplot(122)
            #plt.title("from dataset (ideal h)")
            #plt.imshow(music[i, :, :], cmap=plt.cm.BuPu_r)
            fig.colorbar(img)
            fig.savefig(local_out + "/music_all.eps")
...
out = "figs/NEW_with_only_1_loss_for_piano_v1_DATE_" + str(date.today()) + "_H_" + str(datetime.datetime.now().hour)
out_dir = "./figs/NEW_with_only_1_loss_for_piano_v1_DATE_" + str(date.today()) + "_H_" + str(datetime.datetime.now().hour) + "/"

model_dir = "./figs/NEW_with_only_1_loss_for_E_DATE_2021-05-13_H_12/"
...
parser.add_argument('--batch-size', type=int, default=64,     # changed batch size from 64
...
parser.add_argument('--train_data', type=str, default=base_dir + "music_data_6params_new/train_audio_piano_2s_octlow_piano_8khz_minus2.data",            #train_audio_violin_red_6params.data
                    help='path to training data')

parser.add_argument('--test_data', type=str, default=base_dir + "music_data_6params_new/test_audio_piano_2s_octlow_piano_8khz_minus2.data",
...
parser.add_argument('--train_random_data', type=str, default=base_dir + "music_data_6params_new/train_audio_new3_random_ordered_ed1.data",
...
# # Loading saved model
# E.load_state_dict(torch.load(model_dir + exp_new + 'net/music_model_weight_E1.pt'))
# D.load_state_dict(torch.load(model_dir + exp_new + 'net/music_model_weight_D1.pt'))
...
# E_optimizer = optim.Adam(filter(lambda p: p.requires_grad, E.parameters()), lr=args.encoder_lr)
...
# D_optimizer = optim.Adam(filter(lambda p: p.requires_grad, D.parameters()), lr=args.decoder_lr)

...
class decoder(nn.Module):
    def __init__(self):
        ...
        self.L1 = nn.Sequential(
            nn.Conv1d(6, 256, kernel_size=1),  # changed 1 + 513 * 2 to 16
            nn.BatchNorm1d(256, 256, 1),
            nn.ReLU(),
            #nn.Conv1d(6, 256, kernel_size=1),  # changed 1 + 513 * 2 to 16
            #nn.BatchNorm1d(256, 256, 1),
            #nn.ReLU(),
            nn.Conv1d(256, 256, kernel_size=1),  # changed 1 + 513 * 2 to 16
            nn.BatchNorm1d(256, 256, 1),
            nn.ReLU()
...
def new_training_technique(epoch, train_D=False, train_E=False, init=False):
    ...
    for (batch_idx, data_random), (_, data_train) in zip(enumerate(train_random), enumerate(train_loader)):
        ...
        if train_D and not init:
            ...
            spec_wav = np.zeros((batch_wav_train.shape[0], 128, 126)).astype('float64')
            ...
            for j in range(music_tmp.shape[0]):
                ...
                spec_wav[j, :, 0:125] = np.sqrt(spec_h_temp)[:,0:125]
...
def generate_figures(mode="evaluation", name="", load_weights=("", "")):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        for j in range(music_temp.shape[0]):
            ...
            # spec_tmp[spec_tmp == float("-Inf")] = -50
            # spec_wav[j,:,:] = spec_tmp
            spec_wav[j, :, 0:125] = np.sqrt((nsl.wav2aud(nsl.unitseq(y), paras_c)).T)[:,0:125]
        ...
        # Save ap
        # Save Sp
        music_hat_saved = np.zeros((num, 6, 126), dtype=np.float64)
        # sp_hat_saved = np.zeros((num, 513, 251), dtype=np.float64)
        music_ideal_saved = np.zeros((num, 6, 126), dtype=np.float64)
...
def getSpectrograms(mode="evaluation"):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        spec_wav = np.zeros((batch_wav.shape[0], 128, 126)).astype('float64')
        # spec_wav = np.zeros((batch_wav.shape[0], 128, 250)).astype('float64')
...
parser.add_argument('--train_data', type=str, default=base_dir + "music_data_6params_new/train_audio_1s.data",
                    help='path to training data')

parser.add_argument('--test_data', type=str, default=base_dir + "music_data_6params_new/test_audio_1s.data",
...
parser.add_argument('--train_random_data', type=str, default=base_dir + "music_data_6params_new/train_audio_random_1s.data",
...
E_scheduler = torch.optim.lr_scheduler.ExponentialLR(E_optimizer, gamma=0.05) # changed gamma=0.5 to 0.3
...
D_scheduler = torch.optim.lr_scheduler.ExponentialLR(D_optimizer, gamma=0.05) # changed gamma=0.5 to 0.1
...
trainTogether_newTechnique(epochs=10, init=True, train_D=True, train_E=True, loader_eval="train",
...
for i in range(8): 
    ...
    trainTogether_newTechnique(epochs=50, name="train_E_" + str(i + 1), init=False, train_D=False, train_E=True,
